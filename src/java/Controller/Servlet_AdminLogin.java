            ...
            if (error.length() == 0) {
                ...
                for (int i = 0; i < acc.size(); i++) {
                    if (username.equals(acc.get(i).getUserName()) && password.equals(acc.get(i).getPassword())) {
                        getServletContext().setAttribute("usernameadmin",acc.get(i).getUserName()); // truyen username về jsp
                        ...
                        response.sendRedirect("http://localhost:8084/MXH_Final/AdminMenu.jsp");
                ...
                out.print("window.location = 'http://localhost:8084/MXH_Final/AdminLogin.jsp' ;</script></html>");
                return;
            } else {
                out.print("<html><meta charset=\"utf-8\"/>");
